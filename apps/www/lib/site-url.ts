 ...
 */
const FALLBACK_SITE_URL = "https://ui.testkronus.cloud";

export const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL ?? FALLBACK_SITE_URL).replace(/\/+$/, "");

/**
 * Kronus Pro lives on its own origin (heroui.pro / pro.magicui.design), not
 * `/pro` on this docs chrome. Override with `NEXT_PUBLIC_PRO_URL`.
 */
export const PRO_URL = (
  process.env.NEXT_PUBLIC_PRO_URL ??
  (process.env.NODE_ENV === "development" ? "http://localhost:4748" : "https://kronusui.pro")
).replace(/\/+$/, "");
