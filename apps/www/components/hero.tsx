...
import { WordRotate } from "@kronus-ui/ui/word-rotate";
    ...
    <section id="top" className="relative">
      <div className="mx-auto flex max-w-5xl flex-col items-center px-4 pb-8 pt-16 text-center sm:px-6 sm:pt-24 lg:pt-28">
        <Link
          ...
          className="kronus-rise inline-flex items-center gap-2 rounded-full border border-border bg-surface-raised px-3 py-1 text-xs text-fg-secondary outline-none transition-colors duration-200 ease-[cubic-bezier(.22,1,.36,1)] hover:border-border-strong hover:text-fg focus-visible:ring-2 focus-visible:ring-ring"
        >
        ...
        <h1
          className={`kronus-rise kronus-rise-2 mt-8 w-full font-normal leading-[1.08] tracking-[-0.035em] text-fg text-[2.25rem] sm:text-5xl lg:text-6xl ${displayClassName ?? "font-display"}`}
        >
          ...
          <span className="mt-[0.06em] inline-flex flex-nowrap items-baseline justify-center gap-x-[0.28em] max-sm:flex-wrap">
            ...
            <span className="relative inline-block whitespace-nowrap align-baseline leading-none">
              ...
              <span
                aria-hidden="true"
                className="kronus-word-underline pointer-events-none absolute inset-x-0 h-[3px] rounded-full bottom-[0.1em]"
              />
            </span>
          </span>
        </h1>

        <p className="kronus-rise kronus-rise-3 mt-6 max-w-xl text-pretty text-lg leading-7 text-fg-secondary">
          Tokens, looks, compose, and upgrade. MIT. One command writes a themed SaaS you can keep
          editing.
        </p>

        <div className="kronus-rise kronus-rise-3 mt-10 flex flex-wrap items-center justify-center gap-3">
          <Link
            href="/docs/getting-started"
