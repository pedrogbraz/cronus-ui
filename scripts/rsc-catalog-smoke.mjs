#!/usr/bin/env node
/**
 * rsc-catalog-smoke.mjs — RSC-safety gate for the whole Cronus UI block catalog.
 *
 * Goal: prove that EVERY block in the registry is safe to import into a React
 * Server Component page. The Cronus Compose app generator emits RSC pages that
 ...
 *
 * Approach: import the BUILT dist of create-cronus-app's `scaffold()` (the exact
 * same way compose-smoke does) to lay down the `default` base, then install
 * every block with the SAME machinery the `cronus-ui add` command uses — the
 ...
 * no node_modules to resolve `cronus-ui` from, and lets us point the registry at
 ...
 * the EXACT machinery `cronus-ui add` uses to install a block — reused here so the
...
function ensureBuilt() {
  ...
  for (const f of ["index.json", "meta.json"]) {
    if (!existsSync(join(WORKSPACE_REGISTRY, f))) {
      fatal(
        `registry/${f} is missing — the RSC smoke needs it. Run \`bun run -F cronus-ui registry\` first.`,
      );
    }
  }
}
 ...
 * `npm install`. The published @cronus-ui/* deps are dropped — the base
...
function catalogNpmDeps(Registry, collectDependencies, blocks) {
  return (async () => {
    ...
    const deps = collectDependencies(items).filter((d) => !d.startsWith("@cronus-ui/"));
    return deps.sort();
  })();
}
...
async function buildCatalogApp(dist) {
  ...
  check(existsSync(join(target, "cronus-ui.json")), "scaffolded default base (cronus-ui.json)");
  ...
  //    using the default config (its paths match the scaffolded cronus-ui.json).
  ...
  // components/blocks/<file>.tsx; they import @cronus-ui/ui + lucide-react, not
 ...
 * ------------------------------------------------------------------ */
async function main() {
  log(c.bold("\nCronus UI — RSC catalog smoke runner"));
  log(
    c.dim(
      `mode: ${
        FULL
