 ...
 *                             grouped `cronus.{theme}.{mode}.{token}`, for
 ...
 *                             maps semantic utilities to the runtime --cronus-*
 ...
 * committed CSS and asserts every `--cronus-*` value is value-equivalent to the
...
/** kebab-case the css-var name into the Tailwind theme key (drop `--cronus-`). */
function utilityKey(cssVar: string): string {
  return cssVar.replace(/^--cronus-/, "");
}

/**
 * Tailwind preset. Mirrors the v4 `@theme inline` bridge in tokens.css: every
 * utility resolves to a runtime `var(--cronus-*)` so it re-themes live and never
...
const DTCG_FILE_DESCRIPTION =
  "Cronus UI design tokens in the W3C DTCG format, grouped cronus.{theme}.{mode}.{token}. " +
  "GENERATED by packages/tokens/scripts/build-tokens.ts from src/tokens.ts — do not edit by hand. " +
...
function assertDtcgStructure(cronus: DtcgTree): void {
  const perMode = Object.keys(cssVarMap).length;
  let count = 0;
  for (const theme of themeNames) {
    for (const mode of modes) {
      const group = cronus[theme]?.[mode];
      if (group === undefined) {
        throw new Error(`tokens.dtcg.json: missing group cronus.${theme}.${mode}`);
      }
      const names = Object.keys(group);
      if (names.length !== perMode) {
        throw new Error(
          `tokens.dtcg.json: cronus.${theme}.${mode} has ${names.length} tokens, expected ${perMode}`,
        );
      }
      for (const name of names) {
        ...
        if (!valueOk || !typeOk) {
          throw new Error(`tokens.dtcg.json: cronus.${theme}.${mode}.${name} lacks $value/$type`);
        }
        count += 1;
      }
    }
...
function emitDtcgJson(): string {
  ...
  for (const theme of themeNames) {
    ...
    cronus[theme] = themeGroup;
  }
  assertDtcgStructure(cronus);
  const payload = { $description: DTCG_FILE_DESCRIPTION, cronus };
...
function emitFigmaVariablesJson(): string {
  ...
  const collection: FigmaCollection = { name: "Cronus UI", modes: modeIds, variables };
...
/** All `--cronus-*: value;` declarations inside a single brace-delimited block. */
function parseBlockVars(block: string): Map<string, string> {
  const out = new Map<string, string>();
  const re = /(--cronus-[a-z0-9-]+)\s*:\s*([^;]+);/g;
...
function cascadeLayers(css: string, theme: ThemeName, mode: Mode): CascadeLayers {
  ...
  const baseSelector = `[data-cronus-theme="${theme}"]`;
  ...
  if (mode !== themeBaseMode) {
    const deltaSelector = `[data-cronus-theme="${theme}"][data-cronus-mode="${mode}"]`;
...
async function check(): Promise<void> {
  ...
  const tmp = await mkdtemp(join(tmpdir(), "cronus-tokens-"));
  try {
