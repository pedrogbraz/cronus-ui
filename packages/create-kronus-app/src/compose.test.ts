...
/**
 * `composeTemplate` is a thin adapter over `kronus-ui`'s `composeApp()` library
 ...
 * `registry/meta.json` AND the built `kronus-ui/compose` dist (dynamic import), so
...
const COMPOSE_DIST = fileURLToPath(
  new URL("../node_modules/kronus-ui/dist/compose.js", import.meta.url),
);
const CAN_COMPOSE = existsSync(join(REPO_REGISTRY, "meta.json")) && existsSync(COMPOSE_DIST);

/** A minimal scaffolded project (package.json + kronus-ui.json) for the composer. */
...
describe("composeTemplate — graceful failure (never throws)", () => {
  ...
  it("returns { ok: false } when the target has no kronus-ui.json", async () => {
    ...
    if (!result.ok) expect(result.reason).toMatch(/kronus-ui\.json/);
  });
...
describe.skipIf(!CAN_COMPOSE)("composeTemplate — integration (local repo registry)", () => {
  ...
  it("composes the saas template into (shell)/(bare) route groups with the app-shell chrome", async () => {
    ...
    // surface that create-kronus-app opens on, and it supersedes the base app/page.tsx.
    expect(existsSync(join(cwd, "app/(bare)/login/page.tsx"))).toBe(true);
    expect(existsSync(join(cwd, "app/(shell)/page.tsx"))).toBe(true);
...
/**
 * The wired `create-kronus-app --template store|landing` flow scaffolds the
...
describe.skipIf(!CAN_COMPOSE)("composeTemplate — base app/page.tsx collision (real base)", () => {
  ...
  beforeEach(() => {
    ...
    const configPath = join(cwd, "kronus-ui.json");
