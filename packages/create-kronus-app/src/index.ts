...
export const HELP = `${c.bold("create-kronus-app")} — scaffold a Next.js + Kronus UI app.
...
${c.bold("Examples")}
  ...
  npx create-kronus-app my-app --template landing-studio
  npx create-kronus-app my-app --template mail
  npx create-kronus-app my-app --template dashboard
  npx create-kronus-app my-app --theme sunset --mode light
  npx create-kronus-app my-app --ai --assistants claude,cursor --preset fintech
  npx create-kronus-app my-app --no-ai --pm pnpm --yes
`;
...
async function main(): Promise<void> {
  ...
  const name = parsed.name ?? (await promptName("my-kronus-app"));
  ...
  // Kronus Compose: store/landing scaffold the `default` base above, then generate
  ...
  // scaffold records nothing (matching `kronus-ui add --no-install`).
    ...
    if (composed.ok) {
      ...
      log.warn(
        `Skipped composition (${composed.reason}). The ${c.cyan("default")} app was scaffolded; ` +
          `run ${c.cyan(`npx kronus-ui compose ${template}`)} once the registry ships meta.json.`,
      );
    }
  }
