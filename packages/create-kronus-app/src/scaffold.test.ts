...
describe("create-kronus-app release version", () => {
...
describe("scaffold (into a temp dir, no install)", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "create-kronus-app-"));
  ...
  it("writes package.json with the project name and the three @kronus-ui/* deps", () => {
    ...
    expect(pkg.dependencies["@kronus-ui/ui"]).toBe(`^${CREATE_VERSION}`);
    expect(pkg.dependencies["@kronus-ui/tokens"]).toBe(`^${CREATE_VERSION}`);
    expect(pkg.dependencies["@kronus-ui/theme"]).toBe(`^${CREATE_VERSION}`);
  });
  ...
  it("wires the anti-flash theme script in app/layout.tsx", () => {
    ...
    expect(layout).toContain("KronusThemeScript");
    expect(layout).toContain("KronusUIProvider");
  });
  ...
  it("ships a kronus-ui.json so `npx kronus-ui add` works in the app", () => {
    scaffold({ targetDir, name });
    const configPath = join(targetDir, "kronus-ui.json");
    expect(existsSync(configPath)).toBe(true);
    const config = JSON.parse(readFileSync(configPath, "utf8")) as { registry: string };
    expect(config.registry).toBe(
      `https://raw.githubusercontent.com/pedrogbraz/kronus-ui/v${CREATE_VERSION}/registry`,
    );
  });
  ...
  it("defaults to the aurora/dark theme when none is given (no tokens left behind)", () => {
    ...
    const config = JSON.parse(readFileSync(join(targetDir, "kronus-ui.json"), "utf8")) as {
      theme: { name: string; mode: string };
    };
    expect(config.theme).toEqual({ name: "aurora", mode: "dark" });
  });

  it("bakes the chosen theme + mode into layout.tsx and kronus-ui.json", () => {
  ...
  it.each([
    ...
    expect(existsSync(join(targetDir, "app", "page.tsx"))).toBe(true);
    expect(existsSync(join(targetDir, "kronus-ui.json"))).toBe(true);
