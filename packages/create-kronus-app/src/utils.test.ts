...
describe("HELP", () => {
  it("leads examples with --template saas and documents composed landing flavors", () => {
    expect(HELP).toContain("npx create-kronus-app my-app --template saas");
    expect(HELP).toContain("npx create-kronus-app my-app --template landing-studio");
    expect(HELP).toContain("npx create-kronus-app my-app --template mail");
...
describe("outroLines", () => {
  ...
  it("for composed templates, points at add-page / theme set / upgrade", () => {
    for (const t of ["saas", "store", "landing"] as const) {
      ...
      expect(text, t).toContain("npx kronus-ui theme set aurora --mode dark");
      expect(text, t).toContain("npx kronus-ui upgrade --all --dry-run");
      expect(text, t).not.toContain("npx kronus-ui add dialog table tabs");
    }
  });
  ...
  it("for default/dashboard/marketing, keeps add + a compose hint", () => {
    for (const t of ["default", "dashboard", "marketing"] as const) {
      ...
      expect(text, t).toContain("npx kronus-ui add dialog table tabs");
      expect(text, t).toContain("npx kronus-ui compose saas");
  ...
  it("when template is omitted, keeps the component-add next step", () => {
    ...
    expect(text).toContain("npx kronus-ui add dialog table tabs");
    expect(text).toContain("npx kronus-ui compose saas");
  });
