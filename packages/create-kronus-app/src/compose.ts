 ...
 *
 * `store`/`landing` have no bundled template dir: create-kronus-app copies the
 * `default` base, then this module calls `kronus-ui`'s `composeApp()` library
 ...
 * loading `kronus-ui/compose` — only composed templates reach here.
 *
 * Registry resolution (F1 honest cut — the chicken-and-egg):
 *   A freshly scaffolded app's `kronus-ui.json` pins the registry to the release
 ...
 *     1. `KRONUS_UI_REGISTRY` (env) wins — the dev/CI escape hatch that points at
...
/** What create-kronus-app needs from the composer (a subset of `ComposeAppResult`). */
export interface ComposeTemplateOptions {
  /** The scaffolded project root (contains kronus-ui.json). */
  targetDir: string;
  /** The composed template name (`store` / `landing`). */
  template: string;
  /** Brand string baked into chrome/hero copy (the project name). */
  brand: string;
  /** Skip the composer's own `pm add` (the final create-kronus-app install covers it). */
  skipInstall?: boolean;
  /**
   * Registry override. Defaults to `process.env.KRONUS_UI_REGISTRY` so dev/CI can
 ...
 * page. `create-kronus-app store|landing` scaffolds the `default` base (which
...
export async function composeTemplate(
  ...
  const registry = options.registry ?? process.env.KRONUS_UI_REGISTRY;
  try {
    // Dynamic import: only composed templates load the compose library, and a
    // missing/unbuilt `kronus-ui/compose` degrades to a graceful skip rather than
    // crashing the scaffolder.
    const { composeApp } = await import("kronus-ui/compose");
