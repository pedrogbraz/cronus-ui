 ...
 *
 * Snapshots live at `.kronus-ui/base/<composedKey>/` (the `composed{}` key =
...
export function baseSnapshotDir(appKey: string): string {
  return join(".kronus-ui", "base", appKey);
}
...
/**
 * Read one snapshot file. Prefers `.kronus-ui/base/<composedKey>/<rel>`; if that
 * file is missing, falls back to `.kronus-ui/base/<appName>/<rel>` (legacy
