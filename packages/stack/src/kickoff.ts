/**
 * Kickoff artifacts for a resolved Kronus stack.
 *
 * Turns a {@link StackConfig} into the three things a user leaves the builder
 * with:
 *  - `generateCommand`   → the `bun create kronus-stack@latest …` shell command.
 ...
 * Falls back to "my-kronus-app" when nothing usable remains.
 */
export function sanitizeProjectName(raw: string): string {
  ...
  return slug.length > 0 ? slug : "my-kronus-app";
}
 ...
 */
export function generateCommand(config: StackConfig, projectName = "my-kronus-app"): string {
...
export function generateStackJson(config: StackConfig, projectName = "my-kronus-app"): string {
  const snapshot = {
    $schema: "https://kronus.dev/schema/stack-1.json",
    version: 1,
 ...
 * TRUTH: mission, repo map, commands, configured AI capabilities, the Kronus UI
 * contract (only when UI = Kronus UI), guardrails, and a Definition of Done.
 */
export function generateKickoff(
  ...
  lines.push("");
  lines.push(
    "> Generated by the Kronus Stack Builder. This file is the **single source of truth** for the project's stack, conventions, and mission. Read it fully before writing any code.",
  );
  lines.push("");
  ...
  if (isNext && isKronusUi)
    lines.push("  kronus-ui.json    # Kronus UI registry paths and pinned registry URL");
  if (hasEnv)
    lines.push(
  ...
  // --- Kronus UI contract (only when relevant) ---
  if (isKronusUi) {
    lines.push("## Kronus UI contract");
    lines.push("");
    lines.push("This project uses **Kronus UI**. Treat these as hard rules:");
    lines.push("");
    lines.push(
      "- Use **only** components from `@kronus-ui/ui` — never hand-roll a primitive that already exists.",
    );
    lines.push(
    ...
    lines.push("- Compose class names with `cn` from `@kronus-ui/ui`.");
    ...
    if (multiIds(config, "mcp").includes("mcp-kronus-ui")) {
      lines.push(
        "- The **kronus-ui MCP server** is selected. Use it only after your assistant has loaded the generated `.mcp.json` or an equivalent MCP config.",
      );
    }
    lines.push("");
  }
  ...
  if (isKronusUi)
    lines.push("- [ ] UI uses only `@kronus-ui/ui` + semantic tokens; axe a11y gate is green.");
