...
import { type Ref, Suspense, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
export interface Globe3DLabels {
  /** Accessible name for the globe canvas. @default "Globe" */
  globe: string;
  /** Loading copy shown while textures resolve. @default "Loading globe..." */
  loading: string;
}

const DEFAULT_LABELS: Globe3DLabels = {
  globe: "Globe",
  loading: "Loading globe...",
};

export const globe3dVariants = cva("relative h-[500px] w-full");
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      <mesh position={lineCenter} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial
          color={hovered ? "#ffffff" : "#94a3b8"} // contract-ok: pin stem on the globe the component paints
          transparent
      ...
      <mesh position={surfacePosition} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial
          color={hovered ? "#f97316" : "#ef4444"} // contract-ok: pin tip on the globe the component paints
        />
      </mesh>
...
function RotatingGlobe({ config, markers, onMarkerClick, onMarkerHover }: RotatingGlobeProps) {
  ...
  return (
    <group ref={groupRef}>
      ...
      {markers.map((marker) => (
        <Marker
          key={`${marker.src}-${marker.lat}-${marker.lng}`}
          marker={marker}
  ...
  return (
    <>
      ...
      <directionalLight
        ...
        color="#ffffff" // contract-ok: key light on the globe the component paints
      />
      <directionalLight
        position={[-config.radius * 3, config.radius, -config.radius * 2]}
        intensity={config.pointLightIntensity * 0.3}
        color="#88ccff" // contract-ok: fill light on the globe the component paints
      />
  ...
  return (
    <Html center>
      <div className="flex shrink-0 flex-col items-center gap-3">
        <span className="inline-block shrink-0 text-sm text-fg-tertiary">{label}</span>
      </div>
    </Html>
  );
}
...
const defaultConfig: Required<Globe3DConfig> = {
  radius: 2,
  globeColor: "#1a1a2e", // contract-ok: globe fallback tint the component paints
  textureUrl: DEFAULT_EARTH_TEXTURE,
  bumpMapUrl: DEFAULT_BUMP_TEXTURE,
  showAtmosphere: false,
  atmosphereColor: "#4da6ff", // contract-ok: atmosphere tint the component paints
  ...
  markerSize: 0.06,
  showWireframe: false,
  wireframeColor: "#4a9eff", // contract-ok: wireframe overlay the component paints
...
export function Globe3D({
  ...
  return (
    <div
      ...
      data-slot="globe-3d"
      role="img"
      aria-label={copy.globe}
      className={cn(globe3dVariants(), className)}
    >
      <Canvas
        gl={{
          antialias: true,
      ...
      >
        <Suspense fallback={<LoadingFallback label={copy.loading} />}>
          <Scene
            markers={markers}
            config={mergedConfig}
            onMarkerClick={onMarkerClick}
