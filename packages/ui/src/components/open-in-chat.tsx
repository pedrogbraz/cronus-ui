...
type ProviderKey = "scira" | "chatgpt" | "claude" | "t3" | "v0" | "cursor";

type ProviderDefinition = {
  titleKey: ProviderKey;
...
const DEFAULT_LABELS: OpenInLabels = {
  trigger: "Open in chat",
  scira: "Open in Scira",
...
function useOpenInContext(): OpenInContextValue {
  ...
  if (!context) {
    throw new Error("OpenIn components must be used within an OpenIn");
  }
  return context;
}
...
export type OpenInProps = ComponentProps<typeof DropdownMenu> & {
  query: string;
  labels?: Partial<OpenInLabels>;
};

export function OpenIn({ query, labels: labelsProp, children, ...props }: OpenInProps) {
  const labels = useMemo(() => ({ ...DEFAULT_LABELS, ...labelsProp }), [labelsProp]);
  const value = useMemo(() => ({ query, labels }), [query, labels]);
...
export function OpenInItem(props: OpenInItemProps) {
  return <DropdownMenuItem data-slot="open-in-item" {...props} />;
}
...
export function OpenInLabel(props: OpenInLabelProps) {
  return <DropdownMenuLabel data-slot="open-in-label" {...props} />;
}
...
export function OpenInSeparator(props: OpenInSeparatorProps) {
  return <DropdownMenuSeparator data-slot="open-in-separator" {...props} />;
}
OpenInSeparator.displayName = "OpenInSeparator";

export type OpenInTriggerProps = ButtonProps & {
  labels?: Partial<Pick<OpenInLabels, "trigger">>;
};

export function OpenInTrigger({ children, labels: labelsProp, ...props }: OpenInTriggerProps) {
  const { labels } = useOpenInContext();
  const triggerLabel = labelsProp?.trigger ?? labels.trigger;

  return (
    <DropdownMenuTrigger asChild>
      {children ?? (
        <Button type="button" variant="outline" data-slot="open-in-trigger" {...props}>
          {triggerLabel}
...
function OpenInProviderItem({
  ...
  const { query, labels } = useOpenInContext();
  const definition = providers[provider];
  const title = labels[definition.titleKey];
  const href = definition.createUrl(query);

  return (
    <DropdownMenuItem asChild data-slot={`open-in-${provider}`} {...props}>
      <a href={href} target="_blank" rel="noopener noreferrer">
        <span className="size-4 shrink-0">{definition.icon}</span>
...
OpenInChatGPT.displayName = "OpenInChatGPT";
...
OpenInClaude.displayName = "OpenInClaude";
...
OpenInScira.displayName = "OpenInScira";
...
OpenInCursor.displayName = "OpenInCursor";
