...
function BasicHoverCard() {
  return (
    <HoverCard openDelay={0} closeDelay={0}>
      <HoverCardTrigger href="https://cronus.example">@cronus</HoverCardTrigger>
      <HoverCardContent>
        <p>Cronus — themeable React components.</p>
      </HoverCardContent>
    </HoverCard>
  );
}
...
describe("HoverCard", () => {
  it("renders closed by default", () => {
    ...
    expect(screen.queryByText("Cronus — themeable React components.")).not.toBeInTheDocument();
  ...
  it("opens the card content on pointer hover of the trigger", async () => {
    ...
    await user.hover(screen.getByText("@cronus"));
    // HoverCard content carries no ARIA role (non-modal disclosure), so query by
    // its text / data-slot rather than by role.
    expect(await screen.findByText("Cronus — themeable React components.")).toBeInTheDocument();
  });
  ...
  it("exposes the content via the [data-slot=hover-card-content] surface", async () => {
    ...
    await user.hover(screen.getByText("@cronus"));
    await screen.findByText("Cronus — themeable React components.");
  ...
  it("opens when the trigger receives keyboard focus", async () => {
    ...
    expect(screen.getByText("@cronus")).toHaveFocus();
    expect(await screen.findByText("Cronus — themeable React components.")).toBeInTheDocument();
  });
  ...
  it("closes when the pointer leaves the trigger", async () => {
    const user = userEvent.setup();
    render(<BasicHoverCard />);
    const trigger = screen.getByText("@cronus");
