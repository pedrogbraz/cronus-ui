...
describe("Frame", () => {
  ...
  it("shows the url in the address bar for the browser variant", () => {
    render(
      <Frame variant="browser" url="https://cronus.com/checkout">
        <span>body</span>
      </Frame>,
    );
    expect(screen.getByText("https://cronus.com/checkout")).toBeInTheDocument();
  });

  it("omits the address bar for the window variant", () => {
    render(
      <Frame variant="window" url="https://cronus.com">
        <span>body</span>
      </Frame>,
    );
    expect(screen.queryByText("https://cronus.com")).not.toBeInTheDocument();
  });
  ...
  it("renders three decorative traffic-light dots that are hidden from a11y", () => {
    render(<Frame url="https://cronus.com">body</Frame>);
