...
function sideFacePath(e: BarDepthEntry, topEdge: number, bottomEdge: number): string {
...
export function BarDepthProvider({
  ...
  return <BarDepthContext.Provider value={value}>{children}</BarDepthContext.Provider>;
}
 ...
 */
export function useBarDepthEntries(dataKey: string, activeIndex?: number): BarDepthEntry[] {
  const { data, barScale, bandWidth, yScale, innerHeight, innerWidth, barXAccessor } =
    useChartStable();
        ...
        return (
          ...
          >
            ...
            <g clipPath={`url(#${clipId})`}>
              ...
              <path d={fullSidePath} fill={`url(#${e.isNegative ? sideGlassNeg : sideGlass})`} />
            </g>
      ...
      <defs>
        {activeEntries.flatMap((e) => {
          ...
          return [
            ...
            <linearGradient id={gradId} key={`grad-${safeId}`} x1="0" x2="0" y1="1" y2="0">
              <stop offset="0%" stopColor="white" stopOpacity="0" />
              <stop offset="10%" stopColor="white" stopOpacity="0" />
              <stop offset="22%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY * 0.18} />
              <stop offset="34%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY * 0.5} />
              <stop offset="44%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY * 0.85} />
              <stop offset="50%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY} />
              <stop offset="56%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY * 0.85} />
              <stop offset="66%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY * 0.5} />
              <stop offset="78%" stopColor="white" stopOpacity={PULSE_WAVE_PEAK_OPACITY * 0.18} />
