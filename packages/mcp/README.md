...
| Tool | Input | Does |
| --- | --- | --- |
| `compose_app` | `{ template: "store"\|"landing"\|"saas", brand?: string, dryRun?: boolean, yes?: boolean, skipInstall?: boolean, overwrite?: boolean, variant?: string[] }` | Runs `cronus-ui compose <template> -y` in the project (agents default to `-y`). Optional `--brand`, `--dry-run`, `--skip-install`, `--overwrite`, and one `--variant <slug=id>` per `variant` entry. Prefer this over hand-writing pages when the user wants a full multi-page app from a validated template. Does **not** scaffold a new repo. |
