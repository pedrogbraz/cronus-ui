...
1. **Kronus Pro is its own Next app** (`apps/pro`, port 4748). Aurora dark is
...
2. **See Pro / Explore Pro leave the OSS origin** via `NEXT_PUBLIC_PRO_URL`
   (dev: `http://localhost:4748`, prod name: `https://kronusui.pro`). `/pro` on
   www redirects there so bookmarks still work.
3. **Pricing is Maker ($199) and Studio ($299), one-time perpetual.** Web only.
   No Mobile/Super SKU — Kronus does not ship React Native.
...
Dois `next start` no Playwright. DNS de `kronusui.pro` é deploy, não este
